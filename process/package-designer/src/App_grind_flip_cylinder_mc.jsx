...
const PackageObject = ({ data, parentBaseZ = 0, selectedId, onSelect }) => {
  ...
  let width, depth, centerX, centerZ, radius;
        ...
        <group>
          ...
          {layers.reduce((acc, layer, idx) => {
            ...
            meshes.push(
              ...
              >
                {isCyl ? 
                  <cylinderGeometry args={[radius, radius, layer.thk, 32]} /> : 
...
export default function App() {
  ...
  const getNodeLayerHeight = (node) => node.layers.reduce((sum, l) => sum + l.thk, 0);
  ...
  // --- FILL MC LOGIC (WITH ADOPTION) ---
  const handleFillMC = () => {
    if (!selectedId || selectedId === 'root') {
      alert("Please select an object to base the MC on!");
      return;
    }
    ...
    const userInput = prompt(`Filling MC based on [${baseNode.name}].\nEnter: [Material Name], [Thickness]`, "Epoxy_MC, 15");
    if (userInput === null || userInput.trim() === "") return;

    const parts = userInput.split(',').map(s => s.trim());
    const material = parts[0] || "MC";
    const thickness = parseFloat(parts[1]);
    if (isNaN(thickness)) return;

    const mcZ = baseNode.z + getNodeLayerHeight(baseNode);

    // NEW LOGIC: Wrap siblings that are at or above the baseNode
    const processTree = (curr) => {
      // Look for the level containing the selected node
      if (curr.children && curr.children.some(c => c.id === selectedId)) {
        // Find all siblings that should be "wrapped" (those at or above baseNode.z)
        ...
        const newMC = {
          ...
          layers: [{ material: material, thk: thickness, color: "#2c3e50" }],
          // Adoption: The wrapped siblings become children of the MC
          // We adjust their Z to be relative to the new MC parent
          children: toWrap.map(child => ({
            ...child,
            z: child.z - mcZ 
          }))
        };
      ...
      if (curr.children) {
        return { ...curr, children: curr.children.map(processTree) };
      }
      return curr;
    };
  ...
  const handleFlip = () => {
    const processFlip = (node) => {
      const layerH = getNodeLayerHeight(node);
      return {
        ...node,
        layers: [...node.layers].reverse(),
        children: node.children.map(child => processFlip({ ...child, z: layerH - (child.z + getNodeLayerHeight(child)) }))
      };
    };
  ...
  const calculateMaxZ = (node, currentBaseZ) => {
    ...
    const layerH = getNodeLayerHeight(node);
    let maxZ = myBottom + layerH;
    node.children.forEach(child => { maxZ = Math.max(maxZ, myBottom + calculateMaxZ(child, 0)); });
    return maxZ;
  };

  const handleGrind = () => {
    const globalTopZ = calculateMaxZ(tree, 0);
    const depthStr = prompt(`Top Z: ${globalTopZ.toFixed(2)}\nUnits to grind?`, "5");
    ...
    const processGrind = (node, parentBaseZ) => {
      ...
      node.layers.forEach(layer => {
        if (curr < grindZ) {
          if (curr + layer.thk > grindZ) newLayers.push({ ...layer, thk: grindZ - curr });
          else newLayers.push(layer);
        }
        curr += layer.thk;
      });
      const newChildren = node.children.map(c => processGrind(c, 0)).filter(c => c.layers.length > 0 || c.children.length > 0);
  ...
  const updateNodeRecursive = (id, callback) => {
    const update = (node) => node.id === id ? callback(node) : { ...node, children: node.children.map(update) };
    setTree(update(tree));
  };

  const handleImportChild = (parentId, jsonData) => {
    const reassignIds = (node) => ({ ...node, id: Math.random().toString(36).substr(2, 6), children: node.children ? node.children.map(reassignIds) : [] });
  ...
  const handleRootUpload = (e) => {
    ...
    reader.onload = (evt) => { try { setTree(JSON.parse(evt.target.result)); } catch (err) { alert("Invalid JSON"); } };
    ...
    <div style={{ display: 'flex', width: '100vw', height: '100vh', background: '#0a0a0a', color: '#eee', fontFamily: 'monospace' }}>
      ...
      <div style={{ width: '250px', padding: '15px', borderLeft: '1px solid #333', background: '#111' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button style={{ ...btnMain, background: '#2ecc71', color: '#000' }} onClick={handleFillMC}>🏺 FILL MC 🏺</button>
          <button style={{ ...btnMain, background: '#2ecc71', color: '#000' }} onClick={handleGrind}>⚡ EXECUTE GRIND ⚡</button>
  ...
  return (
    <div style={{ marginLeft: '12px', borderLeft: isSelected ? '2px solid #ff00ff' : '1px solid #444', paddingLeft: '8px', marginBottom: '8px' }}>
      <div onClick={() => onSelect(node.id)} style={{ display: 'flex', alignItems: 'center', background: isSelected ? '#301a30' : 'transparent', padding: '4px', cursor: 'pointer', borderRadius: '2px' }}>
        ...
        />
        <select 
            value={node.face.type} 
            onClick={(e) => e.stopPropagation()}
            onChange={(e) => onUpdateNode(node.id, { face: { ...node.face, type: e.target.value, dim: e.target.value === "CYLINDER" ? [0,0,25] : [0,0,50,50] } })}
