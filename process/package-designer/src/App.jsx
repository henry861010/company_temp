...
const formatForExport = (node) => {
  const { children, ...rest } = node;
  return {
    ...rest,
    Childs: children ? children.map(formatForExport) : []
  };
};

const formatForImport = (node) => {
  const { Childs, children, ...rest } = node;
  const childArray = Childs || children || [];
  return {
    ...rest,
...
export default function App() {
  ...
  const [showGrindForm, setShowGrindForm] = useState(false);
  const [grindParams, setGrindParams] = useState({ depth: 5 });

  const [showRdlForm, setShowRdlForm] = useState(false);
  const [rdlParams, setRdlParams] = useState({ count: 3, metalMat: "Cu", metalThk: 2, pmMat: "PI", pmThk: 3 });

  // Helpers
  const getNodeLayerHeight = (node) => node.layers.reduce((sum, l) => sum + (parseFloat(l.thk) || 0), 0);

  const calculateMaxZ = (node, baseZ) => {
  ...
  // --- DIRECT STRING EXPORT (WITH "Childs" MAPPING) ---
  const handleExport = () => {
    // Convert 'children' to 'Childs' right before stringifying
  ...
  const submitRDL = () => {
    ...
    const rdlChildren = Array.from({ length: count }).map((_, i) => ({
            ...
            <div ref={addMenuRef} style={{ background: '#1a2a33', padding: '8px', margin: '5px 0', borderRadius: '4px', border: '1px solid #00d4ff' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                ...
                <input type="file" ref={fileInputRef} style={{display:'none'}} onChange={(e) => {
                   ...
                   const reader = new FileReader();
                   reader.onload = (evt) => {
                     const importedNode = JSON.parse(evt.target.result);
                     onImportChild(node.id, formatForImport(importedNode));
                   };
                   reader.readAsText(file); e.target.value = null; setShowAddMenu(false);
                }} />
              </div>
            </div>
          )}
...
const btnSmall = { background: '#444', color: '#eee', border: 'none', padding: '4px', fontSize: '9px', cursor: 'pointer', borderRadius: '2px', textAlign:'left' };
...
const coordInput = { background: 'transparent', color: '#00d4ff', border: 'none', fontSize: '10px', width: '32px', outline: 'none' };
const ioBoxStyle = { background: '#1a1a1a', border: '1px solid #333', borderRadius: '6px', padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px' };