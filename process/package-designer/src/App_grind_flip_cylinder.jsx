...
const PackageObject = ({ data, parentBaseZ = 0, selectedId, onSelect }) => {
  ...
  return (
    <group position={[centerX, myBottomZ, centerZ]}>
      ...
      {/* Logic: if parent is cylinder, children are relative to center (0,0). If Box, relative to top-left corner */}
...
export default function App() {
  ...
  const handleFlip = () => {
    const processFlip = (node) => {
      ...
      const flippedChildren = node.children.map(child => {
        ...
        return processFlip({ ...child, z: totalH - (child.z + childH) });
      });
      return { ...node, layers: flippedLayers, children: flippedChildren };
    };
  ...
  const calculateMaxZ = (node, currentBaseZ) => {
    ...
    node.children.forEach(child => {
      maxZ = Math.max(maxZ, myBottom + calculateMaxZ(child, 0));
    });
    return maxZ;
  };
    ...
    const processGrind = (node, parentBaseZ) => {
      ...
      node.layers.forEach(layer => {
        ...
        if (layerBottom < grindPlaneZ) {
          if (layerTop > grindPlaneZ) newLayers.push({ ...layer, thk: grindPlaneZ - layerBottom });
          else newLayers.push(layer);
        }
        currentCumulative = layerTop;
      });
      const newChildren = node.children.map(child => processGrind(child, 0)).filter(child => child.layers.length > 0 || child.children.length > 0);
  ...
  return (
    <div style={{ display: 'flex', width: '100vw', height: '100vh', background: '#0a0a0a', color: '#eee', fontFamily: 'monospace' }}>
      ...
      <div style={{ width: '250px', padding: '15px', borderLeft: '1px solid #333', background: '#111' }}>
        ...
        <button style={{ ...btnMain, background: '#2ecc71', color: '#000', marginBottom: '10px' }} onClick={handleGrind}>⚡ EXECUTE GRIND ⚡</button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {isCyl ? (
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  <div style={coordBox}><label style={labelStyle}>X</label><input style={coordInput} type="number" value={dim[0]} onChange={e => { const val = parseFloat(e.target.value) || 0; onUpdateNode(node.id, { face: { ...node.face, dim: [val, dim[1], val + width, dim[3]] } }); }} /></div>
                  <div style={coordBox}><label style={labelStyle}>Y</label><input style={coordInput} type="number" value={dim[1]} onChange={e => { const val = parseFloat(e.target.value) || 0; onUpdateNode(node.id, { face: { ...node.face, dim: [dim[0], val, dim[2], val + length] } }); }} /></div>
                </div>
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <span style={{ fontSize: '8px', color: '#888', width: '30px' }}>SIZE:</span>
                  <div style={coordBox}><label style={labelStyle}>W</label><input style={coordInput} type="number" value={width} onChange={e => { const val = parseFloat(e.target.value) || 0; onUpdateFace(node.id, 2, dim[0] + val); }} /></div>
                  <div style={coordBox}><label style={labelStyle}>L</label><input style={coordInput} type="number" value={length} onChange={e => { const val = parseFloat(e.target.value) || 0; onUpdateFace(node.id, 3, dim[1] + val); }} /></div>
                </div>
                </>
              )}
