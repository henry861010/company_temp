...
export default function Page() {
  ...
  const submitPnP = async () => {
    ...
    for (const coord of parsedCoords) {
      ...
      const isPoly = newNode.face.type === "POLYGON";

      if (isCyl) {
        newNode.face.dim = [coord.x, coord.y, newNode.face.dim[2]];
      } else if (isPoly) {
        let paths = newNode.face.dim as any[];
        ...
        const dx = coord.x - minX;
        const dy = coord.y - minY;

        newNode.face.dim = paths.map((path: number[][]) => path.map((p: number[]) => [p[0] + dx, p[1] + dy]));
      } else {
  ...
  const calculateUFFace = (gap: number, material: string, targetFaces: Face[]): Face | null => {
    ...
    targetFaces.forEach(face => {
      switch (face.type) {
        ...
        case "POLYGON": {
          const paths = face.dim as number[][][]; // Assuming [pathIndex][pointIndex][x,y]
    ...
    // 4. Map existing boxes as holes
    // Note: For real subtraction, ensure the hole winding order 
    // is opposite to the hull winding order if your renderer requires it.
    const holes = targetFaces
      .filter(f => f.type === "BOX")
      .map(f => {
        const [x1, y1, x2, y2] = f.dim as number[];
        return [[x1, y1], [x2, y1], [x2, y2], [x1, y2]]; // Convert Box to Path
      });
  ...
  const submitFillUF = () => {
    if (!selectedId) return alert("Select an object to base the UF on!");
    const baseNode = findNodeById(tree, selectedId);
    if (!baseNode) return;
    ...
    // Explicit abort if no face was generated
    if (!ufFace) return;
    ...
    if (selectedId === 'root') {
      setTree(prev => ({ ...prev, children: [...prev.children, newUF] }));
    } else {
      ...
      <div style={{ width: '250px', minWidth: '250px', flexShrink: 0, padding: '15px', borderLeft: '1px solid #333', background: '#111', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <button
            ...
            onClick={() => {
              if (!selectedId) return alert("Select a base object to attach the Underfill!");
          ...
          {showPnpForm && (
            <div style={{ background: '#1a2a33', padding: '10px', borderRadius: '4px', border: '1px solid #2ecc71', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <button style={{ ...btnMain, background: '#2ecc71', color: '#fff' }} onClick={submitPnP}>PLACE DIES</button>
            </div>
          )}
